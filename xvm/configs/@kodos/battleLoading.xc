 ...
 */
{
  "battleLoading": {
    ...
    "formatLeft": "<font face='$FieldFont' color='#999999'>{{vehicle}}</font> <font color='{{c:kb}}'>{{kb}}</font> <font color='{{c:wn}}'>{{wn}}</font><font color='{{c:rating}}'>   {{t-rating:3}}</font>",
    // Display format for the right panel (macros allowed, see readme-en.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. readme-ru.txt).
    "formatRight": "<font face='$FieldFont' color='{{c:rating}}'>{{t-rating:3}}   </font><font color='{{c:wn}}'>{{wn}}</font> <font color='{{c:kb}}'>{{kb}}</font> <font color='#999999'>{{vehicle}}</font>"
  }
}
